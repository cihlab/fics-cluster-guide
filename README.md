...
请使用您的学号/工号邮箱进行下列操作（申请完成后您的账号会和您申请所用邮箱绑定）：
...
- `vnc stop` - 停用 VNC 服务；
- `vnc restart` - 重启 VNC 服务（遇到了奇怪的环境或显示问题，不妨先试试这个）；
...
VNC 桌面分辨率的设置和本地的 Linux 桌面设置方法一致，您可以在 VNC 桌面左上角的设置里找。
...
`ilaunch` 在提交任务时，会使用默认的 EDA 环境（不会自动加载您已经 `module load` 的 EDA 软件），您可以参考 `ilaunch` 的文档。
